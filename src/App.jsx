...
function App() {
  ...
  const [manzanasIzquierdas, setManzanasIzquierdas] = useState([]);
  const [manzanasDerechas, setManzanasDerechas] = useState([]);

  const handleclick = () => {

    const aleatorioIzquierdo = Math.round(Math.random() * 9);
    const aleatorioDerecho = Math.round(Math.random() * 9);

    setIzquierda(aleatorioIzquierdo);
    setDerecha(aleatorioDerecho);

    const listazquierda = []
    const listaDerecha = []

    for (let i = 0; i < aleatorioIzquierdo; i++) {
      listazquierda.push("x") 
    }

    for (let i = 0; i < aleatorioDerecho; i++) {
      listaDerecha.push("x")
    }

    setManzanasIzquierdas(listazquierda);
    setManzanasDerechas(listaDerecha);
  }
  ...
  const handleSubmit = (event) => {
    ...
    if ((izquierda + derecha) == respuesta) {

      toast.success("Tu respuesta es correcta, Felicidades!");
      setIzquierda(0);
      setDerecha(0);
      setManzanasIzquierdas([])
      setManzanasDerechas([])
    } else {

      toast.error("Tu respuesta es incorrecta, la proxima lo haras mejor");
      setIzquierda(0);
      setDerecha(0);
      setManzanasIzquierdas([])
      setManzanasDerechas([])
  ...
  return (
    <main className='w-full min-h-screen p-4 flex flex-col gap-4 rounded-lg bg-white/20 text-white'>
     
      <h1 className='text-2xl font-bold'>Frutas y Sumas</h1>
